...
public class ApiController {
    ...
    public ResponseData<TaiSanDto> getTaiSanById(@PathVariable String id) {
        ...
        if (ts == null) {
            return new ResponseData<TaiSanDto>(null); // Should handle 404 properly, but keeping old logic to not break contract
        }
    ...
    public ResponseData<PageResponse<DanhMuc>> getDanhMuc(@PathVariable String loai,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        String loaiDb = loai.toUpperCase().replace("-", "_");
        org.springframework.data.domain.Page<DanhMuc> danhMucPage = danhMucRepo.findByLoai(loaiDb,
